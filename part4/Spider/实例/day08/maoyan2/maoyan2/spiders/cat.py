...
class CatSpider(scrapy.Spider):
    ...
    def parse(self, response):
        movies = response.selector.xpath('//dl[@class="board-wrapper"]/dd')
        for movie in movies:
            maoyan_item = Maoyan2Item()

            movie_div = movie.xpath('.//div[@class="movie-item-info"]')
            maoyan_item['name'] = movie_div.xpath('./p[1]/a/text()').extract_first().strip()
            maoyan_item['actor'] = movie_div.xpath('./p[@class="star"]/text()').extract_first().replace('主演：',
                                                                                                        '').strip()
            maoyan_item['release_time'] = movie_div.xpath('./p[@class="releasetime"]/text()').extract_first().replace(
                '上映时间：', '').strip()
